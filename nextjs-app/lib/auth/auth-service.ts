...
// Local storage keys
const ACCESS_TOKEN_KEY = "token" // Match the keys used in the React app
...
class AuthService {
  ...
  // API Base URL
  private API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8080"

  constructor() {
    console.log("[AuthService] Initializing with API_BASE_URL:", this.API_BASE_URL)

    if (typeof window !== "undefined") {
      // Configure axios interceptors (only in browser)
  ...
  async getValidToken(forceRefresh = false): Promise<string | null> {
    ...
    console.log("[AuthService] Getting valid token, tokens exist:", { token: !!token, refreshToken: !!refreshToken })

    // If no tokens exist, can't proceed
    if (!token || !refreshToken) {
      console.warn("[AuthService] No tokens available")
      return null
    }
  ...
  async logout(): Promise<void> {
    ...
    if (refreshToken) {
      formData.append("refresh_token", refreshToken)
      console.log("[AuthService] Refresh token included in logout request")
    } else {
      console.warn("[AuthService] No refresh token available for logout")
    }

    try {
      console.log("[AuthService] Sending logout request to:", `${this.API_BASE_URL}/api/auth/logout`)

      await axios.post(`${this.API_BASE_URL}/api/auth/logout`, formData.toString(), {
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
        },
      })
