...
class BIService:
    ...
    def get_datasets(self) -> List[Dict]:
        ...
        try:
            with self.postgres_service._get_connection() as conn:
                ...
                # Build the final dataset summaries
                datasets = []
                for row in results:
                    dataset_id = row["id"]
                    ...
                    try:
                        if source_type == "postgres" and query_value and query_value.strip().lower().startswith("select"):
                            ...
                            sample_data = preview_result if isinstance(preview_result, list) else []
                            ...
                            try:
                                minio_config = row["source_config"]

                                bucket_name = minio_config["bucket"]
                                # or similar field
                                object_name = minio_config["object_key"]

                                # Download object content (maybe limit size or use streaming)
                                ...
                                # Parse based on file type (CSV, JSON, etc.)
                                file_format = minio_config.get(
                                    "format", "csv").lower()
                                if file_format == "csv":
                                    sample_data = []
                                ...
                                logger.warning(
                                    f"MinIO preview failed for dataset {dataset_id}: {ex}")
    ...
    def create_dataset(self, dataset: Dict) -> Optional[int]:
        """
        ...
        """
        logger.info(f"Creating new dataset: {dataset.get('name')}")
        try:
            with self.postgres_service._get_connection() as conn:
                # Insert into datasets table
                insert_dataset_query = """
                INSERT INTO datasets (name, description, source_id, query_definition, cache_policy, created_by)
                VALUES (%s, %s, %s, %s, %s, %s)
                RETURNING id
                """
                dataset_params = (
                    ...
                    dataset.get("cache_policy", "none"),
                    dataset.get("created_by", "admin")
                )
                ...
                # Insert associated fields if provided
                fields = dataset.get("schema", {}).get("fields", [])
                if fields:
                    insert_field_query = """
                    INSERT INTO dataset_fields (dataset_id, name, display_name, data_type, field_type, format_pattern)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    """
                    field_params = [
                        (
